...
if (inputForma.news.value.length < 4) {
  document
    .querySelector(".buttom_place_header")
    .classList.remove("buttom_state_activ");
  document.querySelector(".error__text").classList.add("error__text_disabled");
} else {
  document
    .querySelector(".buttom_place_header")
    .classList.add("buttom_state_activ");
  document
    .querySelector(".error__text")
    .classList.remove("error__text_disabled");
}
...
document.forms.Search.addEventListener("submit", function () {
  ...
  if (searchInput.validation(inputForma.news)) {
    ...
    document
      .querySelector(".error__text")
      .classList.add("error__text_disabled");

    inputFieldFlag = true;
    localStorage.setItem("NUMBER_POSITION", 0);
    document
      .querySelector(".preloader")
      .classList.add("preloader_state_enabled");
    //обнуляем страницу
    localStorage.clear();
    arrayUrls.length = 0;
    ...
    document
      ...
      .classList.add("circle-preloader_state_disabled");
    newsCardList.renderCard(
    ...
    newsApi
      ...
      .then((res) => {
        ...
        document
          .querySelector(".body")
          .classList.remove("preloader_error-network");
        document
          .querySelector(".circle-preloader")
          .classList.remove("circle-preloader_state_disabled");
        ...
        document
          .querySelector(".main-top__subtitle")
          .classList.remove("main-top__subtitle_disabled");
      })
      .catch((res) => {
        console.log(res);
        document
          .querySelector(".preloader")
          .classList.remove("preloader_state_enabled");
      });
  }
});
...
document
  .querySelector(".buttom_place_main")
...
if (!JSON.parse(localStorage.getItem("NewsArray"))) {
  document
    .querySelector(".main-top__subtitle")
    .classList.add("main-top__subtitle_disabled");
} else {
  document
    .querySelector(".main-top__subtitle")
    .classList.remove("main-top__subtitle_disabled");
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    const secondArrayLength = JSON.parse(
      localStorage.getItem(localStorage.getItem("NewsName"))
    ).articles.length;
    if (arrayLength >= secondArrayLength) {
      document
        .querySelector(".buttom_place_main")
...
document.querySelector(".cards").addEventListener("click", (event) => {
  ...
  arrayUrls.forEach((element) => {
    if (
      document
        .querySelector("." + ("index" + element["index"]))
...
if (JSON.parse(localStorage.getItem("NewsArray"))) {
  JSON.parse(localStorage.getItem("NewsArray")).forEach((element) => {
    const temporaryArrayNews = JSON.parse(
      localStorage.getItem(localStorage.getItem("NewsName"))
    ).articles[element["index"]];
